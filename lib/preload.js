...
class Preloader extends EventEmitter {
		...
		this.container
			...
			.then((data) => {
				if (data.StatusCode !== 0) {
					const output = this.bufferedStderr
						.getData()
						.toString("utf8")
						.trim();
					let error;
					if (
						...
						error = new this.balena.errors.BalenaError(
							OVERLAY_MODULE_MESSAGE
						);
					} else {
	...
	_getCommit() {
		return (
			this.commit || this.application.should_be_running__release[0].commit
		);
	}
		...
		if (this._supervisorLT7()) {
			...
			_.each(images, (image) => {
				image.is_stored_at__image_location = _.find(
					servicesImages,
					(serviceImage) => {
						return serviceImage.startsWith(
	...
	_getImagesToPreload() {
		...
		for (const image of toPreload) {
			if (
				preloaded.has(image.is_stored_at__image_location.split("@")[0])
			) {
		...
		for (const manifest of manifests) {
			for (const layer of manifest.manifest.layers) {
				if (!digests.has(layer.digest)) {
					...
					sizeRequests.push({
						imageLocation: manifest.imageLocation,
						layer,
					});
				}
			}
	...
	async _getApplicationSize() {
		...
		const layersSizes = await this._getLayersSizes(
			manifests,
			registryToken
		);
		let extra = 0;
	...
	prepare() {
		return Bluebird.resolve(this._build()).then(async () => {
			...
			const port = await this._runWithSpinner(
