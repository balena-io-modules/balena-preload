...
const createContainer = (docker, image, splashImage, dockerPort, proxy, edisonFolder) => {
  ...
  .then((version) => {
    if (os.platform() === 'linux') {
      // On a Fedora 25 host, loop devices created by losetup in the container appear in
      // the host but not in the container. This bind mount works around the issue.
