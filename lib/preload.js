 ...
 */
const pullBuild = (stdout, stdin, dockerPort, imageId) => {
  stdout.on('data', (data) => {
    ...
    const progress = new visuals.Progress(`Pulling image`)
    innerDockerProgress.pull(imageId, (event) => {
      progress.update({ percentage: event.percentage })
    })
    .then(() => {
...
const runPreloadContainerAndPull = (docker, container, imageId, dockerPort) => {
  ...
  .then((container) => {
    ...
    .then((stream) => {
      ...
      pullBuild(stdout, stream, dockerPort, imageId)
...
const checkImage = (resin, image) => {
  return isReadWriteAccessibleFile(image)
  .then((ok) => {
    if (!ok) {
      throw new resin.errors.ResinError('The image must be a read / write accessible file')
    }
  })
}
...
const getContainerSize = (apiHost, apiToken, registryHost, imageRepo) => {
  const progress = new visuals.Progress(`Fetching container size for ${imageRepo}`)
  progress.update({ percentage: 0 })
  ...
  .then((token) => {
    ...
    .then((response) => {
      ...
      const updateProgress = () => {
        progress.update({ percentage: finished / size * 100 })
      }
      updateProgress()
      return Promise.map(
...
preload.build = (docker) => {
  ...
  const progress = new visuals.Progress('Building Docker preloader image.')
  progress.update({ percentage: 0 })
    ...
    docker.modem.followProgress(
      ...
      (event) => {  // onProgress
        if (event.stream) {
          ...
          if (matches) {
            progress.update({ percentage: parseInt(matches[1]) / (parseInt(matches[2]) + 1) * 100 })
          }
...
preload.run = (resin, docker, options) => {
  ...
  return checkImage(resin, options.image)
  .then(() => {
  ...
  .then((token) => {
    ...
    return Promise.all([
      getContainerSize(options.apiHost, options.apiToken, options.registryHost, options.imageRepo),
      getPort()
    ])
  })
  .spread((containerSize, dockerPort) => {
    options.containerSize = containerSize
    options.dockerPort = dockerPort
    return createContainer(docker, options)
  })
  .then((container) => {
    return runPreloadContainerAndPull(docker, container, options.appData.imageId, options.dockerPort)
  })
}
 ...
 */
preload.getDeviceTypeSlugAndPreloadedBuilds = (resin, docker, options) => {
  return checkImage(resin, options.image)
  .then(() => {
