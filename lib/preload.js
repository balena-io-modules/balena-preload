...
const isReadWriteAccessibleFile = (image) => {
  return Bluebird.join(fs.promises.access(image, R_OK | W_OK), fs.promises.stat(image), (_, stats) => {
    return stats.isFile()
  })
  .catchReturn(false)
}
...
class Preloader extends EventEmitter {
  constructor (balena, docker, appId, commit, image, splashImage, proxy, dontCheckArch, pinDevice = false, certificates = []) {
    super()
    if (balena == null) {
  ...
  _unzipFiles (archive, folder) {
    ...
    let position = 0
    this._progress(name, 0)
    return Bluebird.resolve(fs.promises.stat(archive))
    .then((stat) => {
  ...
  prepare () {
    ...
    .then(() => {
      ...
      return this._runWithSpinner('Finding a free tcp port and getting balena settings', () => {
        return Bluebird.all([getPort(), this.balena.settings.getAll(), this.balena.auth.getToken()])
      })
    })
    ...
    .then(() => {
      return Bluebird.each(this.certificates, (certificate) => {
        return this.container.putArchive(
          tarfs.pack(path.dirname(certificate), { entries: [ path.basename(certificate) ] }),
          {
  ...
  cleanup () {
    ...
    return this._runWithSpinner('Cleaning up temporary files', () => {
      return Bluebird.try(() => {
        if (this.container) {
          return Bluebird.all([this.kill(), this.container.wait()])
          .then(() => {
  ...
  _getAppData () {
    if (this._supervisorLT7()) {
      ...
      return _.merge(_.omit(this.state.local, 'name'), { pinDevice: this.pinDevice })
    }
  }
