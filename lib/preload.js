...
class Preloader extends EventEmitter {
		...
		return Bluebird.resolve(
			...
			.then(() => {
				return this.balena.request.send({
					...
					url: `device/v${this._supervisorLT7() ? 1 : ${this._supervisorLT13() ? 3 : 2}}/${uuid}/state`,
				});
			})
			.get('body')
			.then((state) => {
	...
	_supervisorLT13() {
		try {
			...
			// Suppose the supervisor version is >= 13.0.0 when it is not valid semver.
			return false;
		}
	}
	...
	_getAppData() {
		if (this._supervisorLT7()) {
			...
			// We have read a v2 state because the SV is 7 < n < 13
			// Should we do something here or all is fine?
		} else {
...
preload.applicationExpandOptions = {
	owns__release: {
		$select: ['uuid', 'id', 'commit', 'end_timestamp', 'composition'],
		$orderby: [{ end_timestamp: 'desc' }, { id: 'desc' }],
		$expand: {
			contains__image: {
				$select: ['image'],
