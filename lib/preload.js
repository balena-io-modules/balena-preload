...
class Preloader extends EventEmitter {
  ...
  prepare () {
    ...
    .then(() => {
      return Promise.each(this.certificates, (certificate) => {
  ...
  setAppIdAndCommit (appId, commit) {
    console.error(`setAppIdAndCommit: setting appId=${appId} commit=${commit}`);
    this.appId = appId
    this.commit = commit
    this.application = null
    return this._fetchApplication().finally(() => {
      console.error(`setAppIdAndCommit: done: this.appId=${this.appId} this.commit=${this.commit}`);
    })
  }
}
