...
const R_OK = _.isUndefined(fs.constants) ? fs.constants.R_OK : fs.R_OK
const W_OK = _.isUndefined(fs.constants) ? fs.constants.W_OK : fs.W_OK
...
// Standard resinOS image partitions
const PARTITIONS = {
  boot: {
    image: '/img/resin.img',
    number: 1
  ...
  conf: {  // only needed for resinOS 1.8 images
    image: '/img/resin.img',
    number: 5
  },
  data: {
...
const EDISON_BOOT_PARTITION_FILENAME = 'resin-image-edison.hddimg'
const EDISON_ROOT_PARTITION_FILENAME = 'resin-image-edison.ext3'
const EDISON_DATA_PARTITION_FILENAME = 'resin-data.img'
const EDISON_PARTITIONS = {
  boot: {
    image: `/img/${EDISON_BOOT_PARTITION_FILENAME}`,
    number: 0
  },
  root: {
    image: `/img/${EDISON_ROOT_PARTITION_FILENAME}`,
    number: 0
  },
  data: {
    image: `/img/${EDISON_DATA_PARTITION_FILENAME}`,
    number: 0
  }
}
...
const isEdisonZipArchive = (file) => {
  return zipContainsFiles(
    file,
    [EDISON_BOOT_PARTITION_FILENAME, EDISON_ROOT_PARTITION_FILENAME, EDISON_DATA_PARTITION_FILENAME]
  )
  .catchReturn(false)
}

class Preloader extends EventEmitter {
  constructor (resin, docker, appId, commit, image, splashImage, proxy, dontDetectFlasherTypeImages) {
    super()
    this.resin = resin
    this.docker = docker
    this.appId = appId
    ...
    this.proxy = proxy
    this.dontDetectFlasherTypeImages = dontDetectFlasherTypeImages
    this.imageRepo = null
  ...
  _getContainerSize () {
    const name = `Fetching container size for ${this.imageRepo}`
    this._progress(name, 0)
    return getRegistryToken(this.resin.pine.API_URL, this.token, this.resinSettings.registry2Url, this.imageRepo)
    .then((registryToken) => {
      return registry(this.resinSettings.registry2Url, `v2/${this.imageRepo}/manifests/latest`, registryToken, {}, true, true)
      .then((response) => {
        ...
        return Promise.map(
          ...
          (layer) => {
            return getLayerSize(this.resinSettings.registry2Url, registryToken, this.imageRepo, layer[0])
            .then((layerSize) => {
              finished += 1
              updateProgress()
    ...
    .then((containerSize) => {
      ...
      return this._runWithSpinner('Resizing partitions and waiting for dockerd to start', () => {
        return this._runCommand('preload', {
          ...
          detect_flasher_type_images: !this.dontDetectFlasherTypeImages
        })
      })
    })
    .then(() => {
  ...
  setApplication (application) {
    ...
    this.imageRepo = getImageRepo(this.application, this.commit)
  }
...
const createContainer = (docker, image, splashImage, dockerPort, proxy, edisonFolder) => {
  ...
  if (splashImage) {
    mounts.push(bindMount(splashImage, SPLASH_IMAGE_PATH_IN_DOCKER))
  }
  if (edisonFolder) {
    partitions = EDISON_PARTITIONS
    mounts.push(
      bindMount(path.join(edisonFolder, EDISON_BOOT_PARTITION_FILENAME), EDISON_PARTITIONS.boot.image),
      bindMount(path.join(edisonFolder, EDISON_ROOT_PARTITION_FILENAME), EDISON_PARTITIONS.root.image),
      bindMount(path.join(edisonFolder, EDISON_DATA_PARTITION_FILENAME), EDISON_PARTITIONS.data.image)
    )
  } else {
    partitions = PARTITIONS
    mounts.push(bindMount(image, DISK_IMAGE_PATH_IN_DOCKER))
  }
