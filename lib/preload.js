...
class Preloader extends EventEmitter {
	...
	async _fetchDeviceTypes() {
		const dt = await this.balena.models.config.getDeviceTypes();
		this.deviceTypes = dt;
		return dt;
	}
	...
	async _getState() {
		...
		const { body: state } = this.balena.request.send({
	...
	prepare() {
		return Bluebird.resolve(this._build()).then(async () => {
			...
			await this._fetchDeviceTypes();
			await this._fetchApplication();
			return await this._getImageInfo();
		});
	}
