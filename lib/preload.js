...
class Preloader extends EventEmitter {
  constructor (resin, docker, appId, commit, image, splashImage, proxy, dontCheckArch, pinDevice = false) {
    super()
    this.resin = resin
    this.docker = docker
    this.appId = appId
  ...
  _fetchDeviceTypes () {
    return this.resin.models.config.getDeviceTypes()
    .tap((dt) => {
      this.deviceTypes = dt
    })
  }
  ...
  _deviceTypeArch (slug) {
    ...
    if (deviceType === undefined) {
      throw new this.resin.errors.ResinError(`No such deviceType: ${slug}`)
    }
  ...
  _ensureCanPreload () {
    ...
    if (this.dontCheckArch === false && imageArch !== applicationArch) {
  ...
  _getAppData () {
    if (this._supervisorLT7()) {
      ...
      return _.map(this.state.local.apps, (value, appId) => {
        return _.merge({}, _.omit(value, [ 'environment', 'image' ]), { appId, env: value.environment, imageId: value.image })
      })
    } else {
