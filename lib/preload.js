...
class Preloader extends EventEmitter {
  ...
  _getRelease () {
    ...
    return _.find(this.application.owns__release, { commit })
  }
  ...
  _getComposition () {
    return JSON.parse(this._getRelease().composition)
  }
  ...
  _supervisorLT7 () {
    try {
      ...
      // FIXME: Multicontainer supervisor versions are not relaesed yet.
      return false
    }
  }
  ...
  cantPreload () {
    // Returns an error message if preloading is not possible.

    // No releases
    if (this.application.owns__release.length === 0) {
      return 'This application has no successful releases'
    }
    ...
    if (this.dontCheckDeviceType === false && this.config.deviceType !== this.application.device_type) {
      return `Application device type (${this.application.device_type}) and image device type (${this.config.deviceType}) do not match.`
    }
    ...
    if ((this._getImages().length > 1) && this._supervisorLT7()) {
      return `Can't preload a multicontainer app on an image which supervisor version is < 7.0.0 (${this.supervisorVersion}).`
    }
  ...
  preload () {
    ...
    .then((registryToken) => {
      ...
      return Promise.each(images, (image, index) => {
        const pullingProgressName = `Pulling image ${index + 1}/${images.length} ${image}`
        ...
        // Pull the image
        return innerDockerProgress.pull(image, progressHandler, opts)
      })
    })
    .then(() => {
