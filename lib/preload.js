...
const getContainerSize = (apiHost, apiToken, registryHost, imageRepo, fetchSizeProgress) => {
  fetchSizeProgress.emit('progress', 0)
  ...
  .then((token) => {
    ...
    .then((response) => {
      ...
      const updateProgress = () => {
        finished += 1
        fetchSizeProgress.emit('progress', finished / size * 100)
      }
      updateProgress()
      return Promise.map(
...
preload.run = (stdin, resin, docker, options) => {
  ...
  .then((token) => {
    ...
    return Promise.all([
      getContainerSize(options.apiHost, options.apiToken, options.registryHost, options.imageRepo, fetchSizeProgress),
      getPort()
    ])
  })
