...
import type { Application, BalenaSDK, DeviceType, PineDeferred, PineFilter, Release } from "balena-sdk"
...
async function getEdisonPartitions(edisonFolder): Promise<{ [name: string]: { file: string; image: string } }> {
  // The replace is needed because this file contains new lines in strings, which is not valid JSON.
  const data = JSON.parse((await fs.readFile(path.join(edisonFolder, FLASH_EDISON_FILENAME), "utf8")).replace(/\n/g, ""))
...
function setBindMount(hostConfig: Docker.HostConfig, mounts: Array<[string, string]>, dockerApiVersion: string) {
...
const createContainer = async (docker: Docker, image: string, splashImage: string, dockerPort: number, proxy: string, edisonFolder: string) => {
  ...
  const env = [`HTTP_PROXY=${proxy || ""}`, `HTTPS_PROXY=${proxy || ""}`, `DOCKER_PORT=${dockerPort || ""}`, `DOCKER_TMPDIR=${DOCKER_TMPDIR}`]
...
export class Preloader extends EventEmitter {
      ...
      this.docker.modem.followProgress(
        ...
        (event) => {
          ...
          if (event.stream) {
            ...
            if (matches) {
              this._progress(name, (parseInt(matches[1], 10) / (parseInt(matches[2], 10) + 1)) * 100)
            }
    ...
    return new Bluebird((resolve, reject) => {
      ...
      this.stdout.once("data", (data) => {
        ...
        if (response.statusCode === 0) {
          ...
          const msg = [`An error has occurred executing internal preload command '${command}':`, cmd]
          if (response.error) {
  ...
  async registry(registryUrl, endpoint, registryToken, headers, decodeJson, followRedirect, encoding?) {
    headers = { ...headers }
  ...
  async _getLayerSize(token, registryUrl, layerUrl) {
    ...
    let response = await this.registry(registryUrl, layerUrl, token, headers, false, false, null)
  ...
  async _getApplicationSize() {
    ...
    for (const { imageLocation, manifest } of manifests) {
      ...
      const size = _.sumBy(manifest.layers, (layer: Layer) => layersSizes.get(layer.digest).size)
  ...
  async _fetchApplication() {
    ...
    return this._runWithSpinner(`Fetching application ${this.appId}`, async () => {
      const releaseFilter: PineFilter<Release> = {
        status: "success",
      }
      if (this.commit === "latest") {
        const { should_be_running__release } = await this.balena.models.application.get(this.appId, {
          $select: "should_be_running__release",
        })
        // TODO: Add a check to error if the application is not tracking any release
        releaseFilter.id = (should_be_running__release as PineDeferred | null)!.__id
      } else if (this.commit != null) {
        releaseFilter.commit = { $startswith: this.commit }
      }

      const application = await this.balena.models.application.get(this.appId, {
        $expand: {
  ...
  prepare() {
    return Bluebird.resolve(this._build()).then(async () => {
      ...
      const isEdison = await this._runWithSpinner("Checking if the image is an edison zip archive", () => isEdisonZipArchive(this.image))
      ...
      const container = await this._runWithSpinner("Creating preloader container", () =>
        createContainer(this.docker, this.image, this.splashImage, this.dockerPort, this.proxy, this.edisonFolder)
      )
      this.container = container
      await this._runWithSpinner("Starting preloader container", () => this.container.start())
      ...
      await Promise.all([this._getImageInfo(), this._fetchDeviceTypes(), this._fetchApplication()])
    })
  }
  ...
  preload() {
    return Bluebird.resolve(this._getState()).then(async () => {
      ...
      await this._runWithSpinner("Resizing partitions and waiting for dockerd to start", () =>
